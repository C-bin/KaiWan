...
- (void)creatUI {
    ...
    [aview makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.equalTo(HeightScale(96));
    }];
    ...
    UILabel *titleLabel1 = [UILabel creatLabelWithFont:15 andbgcolor:nil andtextColor:SF_COLOR(51, 51, 51) andAligment:0];
    titleLabel1.text = @"姓名  ";
    ...
    NSString *str1 = @"请输入你的真实姓名";
    NSMutableAttributedString *mstr1 = [[NSMutableAttributedString alloc]initWithString:str1];
    [mstr1 addAttribute:NSForegroundColorAttributeName value:SF_COLOR(220, 220, 220) range:NSMakeRange(0, str1.length)];
    [mstr1 addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:15] range:NSMakeRange(0, str1.length)];
    self.miMaTf.attributedPlaceholder = mstr1;
    self.miMaTf.font = [UIFont systemFontOfSize:15];
    [aview addSubview:self.miMaTf];
    
    [self.miMaTf makeConstraints:^(MASConstraintMaker *make) {
        make.top.bottom.equalTo(titleLabel1);
        make.left.equalTo(titleLabel1.right);
        make.right.equalTo(0);
    }];
    ...
    NSString *str11 = @"1.提现将于次日22:00前到账（周末节假日不打款）请耐心等待；\n2.如提现失败，提现金额将于48小时内退回。";
    ...
    [attrStr addAttribute:NSParagraphStyleAttributeName value:paragraphStyle1 range:NSMakeRange(0, [str1 length])];
